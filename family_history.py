...
def print_death_age(people_dict):
    ...
    for person in people_dict.items():
        # v of the key in the k,v person.
        values = person[1]
        print(f"Name: {values[0]}, Age at Death: {values[3] - values[2]}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage in marriages_dict.items():
        values = marriage[1]
        husband_key = values[HUSBAND_KEY_INDEX]
        wife_key = values[WIFE_KEY_INDEX]
        wedding_year = values[WEDDING_YEAR_INDEX]
        husband_age = 0 
        wife_age = 0 
        husband_name = ""
        wife_name = ""
        if husband_key in people_dict:
            info = people_dict[husband_key]
            husband_name = info[NAME_INDEX]
            husband_age = wedding_year - info[BIRTH_YEAR_INDEX]
        if wife_key in people_dict:
            info = people_dict[wife_key]
            wife_name = info[NAME_INDEX]
            wife_age = wedding_year - info[BIRTH_YEAR_INDEX]
